...
class ModuleLoader(Module):
    ...
    helpText = u"load/reload <command>, unload <command> - handles loading/unloading/reloading of commands. " \
    ...
    def onTrigger(self, message):
        ...
        if len(message.parameterList) == 0:
            return IRCResponse(ResponseType.PRIVMSG, u"You didn't specify a module name!", message.user, message.replyTo)
        ...
        for moduleName in message.parameterList:
            ...
            if message.command.lower() == u"unload":
                if moduleName.lower() in self.bot.moduleHandler.loadedModules:
                    ...
                    returns[moduleName] = False, u"No module named \"{}\" is loaded!".format(moduleName)
                    log.err("Tried to unload module \"{}\" but it is not loaded!".format(moduleName))
        ...
        if len(failures) == 0:
            ...
            return IRCResponse(ResponseType.PRIVMSG, u"{} failed to load.".format(u", ".join(failures)), message.user, message.replyTo)
        else:
            return [IRCResponse(ResponseType.PRIVMSG, u"{} loaded successfully!".format(u", ".join(successes)), message.user, message.replyTo),
